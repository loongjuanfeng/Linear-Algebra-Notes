...
$norm(dot)_1$ 衡量的是所有坐标偏差的总量，因此对稀疏结构很敏感；$norm(dot)_2$ 就是通常的欧氏长度，最符合几何直觉；$norm(dot)_oo$ 只看最坏的那个坐标，因此常用于控制逐坐标误差。

#caveat[“$norm(dot)_0$” 不是范数][
...
#connection[单位球反映了范数的几何][
  ...
  + $norm(dot)_2$ 的单位球是圆盘
  + $norm(dot)_1$ 的单位球是菱形
  + $norm(dot)_oo$ 的单位球是正方形
]

这件事很值得记住。选择范数，不只是换一个公式而已，而是在改变空间里的几何。最优化问题中的约束区域、最近点的形状、误差传播的方式，都会随着范数变化而变化。

#theorem[有限维空间中的范数等价][
  设 $V$ 是有限维实向量空间，$norm(dot)_a$ 与 $norm(dot)_b$ 是 $V$ 上两个向量范数，则存在常数 $c, C > 0$，使得对任意 $vc(x) in V$ 都有
  $
...
先选定 $V$ 的一组基，把 $V$ 与 $RR^n$ 识别，并以 $norm(dot)_2$ 表示欧氏范数。由反三角不等式可知，每一个范数都是连续函数，因为
$
...
$
它是闭且有界的，因此是紧集。连续函数 $norm(dot)_a$ 在紧集 $S$ 上必能取到最小值与最大值，记为 $m_a$ 与 $M_a$。又因为 $vc(x) in S$ 时不可能有 $vc(x) = 0$，所以 $m_a > 0$。于是对任意 $vc(u) in S$，都有
$
...
$
对 $norm(dot)_b$ 也同样成立，于是二者都被欧氏范数夹住，自然就能彼此比较，得到存在常数 $c, C > 0$ 使得
$
  c norm(vc(x))_a <= norm(vc(x))_b <= C norm(vc(x))_a
$
...
这就是 $p$ 范数的三角不等式。前面虽然已经把 $norm(dot)_p$ 当作常见范数使用，但真正支撑这一点的正是Minkowski不等式。
...
第二部分说明怎样从几何形状反推出范数。定义中的 $norm(dot)_K$ 称为 $K$ 的Minkowski泛函，也叫规范函数。它的意义是：从原点出发，沿着向量 $vc(x)$ 的方向走出去，要把单位凸体 $K$ 放大多少倍，才刚好能碰到 $vc(x)$。因此 $norm(vc(x))_K$ 不是在“算公式”，而是在测量 $vc(x)$ 相对于几何体 $K$ 的径向位置。

这个定义为什么自然，可以直接从几个简单形状看出来：

+ 若 $K$ 是欧氏圆盘，那么 $norm(dot)_K$ 就是 $2$ 范数。
+ 若 $K$ 是菱形，那么 $norm(dot)_K$ 就是 $1$ 范数。
+ 若 $K$ 是与坐标轴平行的正方形，那么 $norm(dot)_K$ 就是 $oo$ 范数。
...
#connection[$p$ 范数确实满足三条公理][
  对 $1 <= p <= oo$，函数 $norm(dot)_p$ 的非负性与齐次性都是直接可见的，而三角不等式正由Minkowski不等式给出。因此 $RR^n$ 上的 $p$ 范数确实都是合法的向量范数。
]
...
换个角度看，同一个向量如果有很多个中等大小的坐标，那么 $norm(dot)_1$ 往往明显大于 $norm(dot)_2$，而 $norm(dot)_2$ 又往往大于 $norm(dot)_oo$；如果向量几乎只有一个坐标特别大，其余都很小，那么这几种范数就会彼此接近。因此不同范数的差异，本质上反映的是“能量分布得有多分散”。
...
#definition[诱导矩阵范数][
  设 $norm(dot)_a$ 是 $RR^n$ 上的向量范数，$norm(dot)_b$ 是 $RR^m$ 上的向量范数。对任意 $mt(A) in RR^(m times n)$，定义
  $
  ...
  当 $m = n$ 且定义域、值域采用同一个向量范数 $norm(dot)_p$ 时，常记为
  $
...
#intuition[诱导范数的几何意义][
  ...
  $
    norm(mt(A))_((b,a))
  $
  就是这个像集被 $norm(dot)_b$ 度量时离原点最远的距离。
]
...
#proposition[诱导范数的基本性质][
  ...
  若 $mt(A), mt(B) in RR^(n times n)$ 且都由同一个向量范数 $norm(dot)_a$ 诱导，则
  $
...
#caveat[不是每个矩阵范数都与乘法相容][
  ...
  例如最大元素范数 $norm(dot)_"max"$ 一般只满足带维数因子的估计，而不一定满足常数恰好为 $1$ 的次乘性。因此当我们关心矩阵连乘、幂迭代、稳定性与误差传播时，通常更偏爱诱导范数或Frobenius范数。
]
...
#connection[矩阵范数之间同样等价][
  因为 $RR^(m times n)$ 也是有限维向量空间，所以任意两个矩阵范数也是等价的。也就是说，对任意两个矩阵范数 $norm(dot)_a$ 与 $norm(dot)_b$，存在常数 $c, C > 0$，使得
  $
...
#definition[对偶范数][
  设 $norm(dot)_a$ 是 $RR^n$ 上的向量范数。定义它的对偶范数为
  $
...
$
  norm(dot)_(p^*) = norm(dot)_q
$
也就是说，前一节里的Hölder不等式，本质上就是“线性函数在单位球上的最大值由对偶范数给出”。

#theorem[诱导矩阵范数的对偶表示][
  设 $norm(dot)_a$ 是 $RR^n$ 上的范数，$norm(dot)_b$ 是 $RR^m$ 上的范数，它们的对偶范数分别记为 $norm(dot)_(a^*)$ 与 $norm(dot)_(b^*)$。则对任意 $mt(A) in RR^(m times n)$，都有
  $
...
#theorem[次乘性、幂与条件数的第一层估计][
  设 $norm(dot)$ 是一个与矩阵乘法相容的矩阵范数，即满足
  $
    norm(mt(A) mt(B)) <= norm(mt(A)) norm(mt(B))
  $
  则对任意正整数 $k$ 都有
...
#theorem[谱半径受次乘性矩阵范数控制][
  设 $norm(dot)$ 是任意次乘性矩阵范数，$rho(mt(A))$ 表示 $mt(A)$ 的谱半径，则
  $
    rho(mt(A)) <= norm(mt(A))
  $
]
