...
前面已经见过不少范数估计，但真正把 $p$ 范数理论串起来的是几条经典不等式。它们有一条很清楚的逻辑链：先用 Young 不等式控制两个非负数的乘积，再推广成 Hölder 不等式来控制求和，接着由此推出 Cauchy-Schwarz 不等式与 Minkowski 不等式，最后得到不同 $p$ 范数之间的系统比较。

如果从“要控制什么量”这个角度看，这一串不等式其实对应三种最基本的运算：

+ Young 不等式处理单个乘积。
+ Hölder 不等式处理一串乘积的求和。
+ Minkowski 不等式处理两个向量的相加。
...
$
  a b - a^p / p <= b^q / q
$
也就是 Young 不等式。等号成立恰好对应于极大点条件 $a = b^(q - 1)$，等价于 $a^p = b^q$。
...
$
两边再乘上 $norm(vc(x))_p norm(vc(y))_q$，便得到 Hölder 不等式。

从证明也能看出它的内核：先把两个向量都归一化到各自单位球面上，再证明归一化后的逐项乘积总和不超过 $1$。因此 Hölder 不等式本质上是在说：互为共轭的两个单位球之间，存在一种恰好匹配的配对规则。

几何上还可以把它看成“支撑超平面”的问题。固定 $vc(y)$ 以后，集合
$
  {vc(x) in RR^n : sum_(i = 1)^n x_i y_i = c}
$
是一族彼此平行的超平面。随着 $c$ 增大，这族超平面向外平移；而 Hölder 不等式说明，当 $vc(x)$ 被限制在 $p$ 单位球内时，能够碰到的最大 $c$ 正好是 $norm(vc(y))_q$。也就是说，$q$ 范数正是在测量这个超平面能把 $p$ 单位球“顶”到多远。

#corollary[Cauchy-Schwarz 不等式][
...
它只是 Hölder 不等式在 $p = q = 2$ 时的特例，但因为最常用，所以通常单独命名。在线性代数里，几乎所有与内积、投影、正交分解有关的基本估计，都会反复用到它。

几何上，Cauchy-Schwarz 不等式说明：两个向量的内积不会超过它们长度的乘积。也就是说，内积真正测到的是“长度乘上方向对齐程度”。当两个向量同向或反向时，对齐程度最大，于是最接近等号；当它们正交时，内积则降到 $0$。

在欧氏空间里，这一点还能写成熟悉的公式
$
  vc(x)^"T" vc(y) = norm(vc(x))_2 norm(vc(y))_2 cos theta
$
所以 Cauchy-Schwarz 不等式其实只是
$
  abs(cos theta) <= 1
$
...
这就是 $p$ 范数的三角不等式。前面虽然已经把 $norm(·)_p$ 当作常见范数使用，但真正支撑这一点的正是 Minkowski 不等式。

如果把 $norm(vc(x))_p$ 看成向量 $vc(x)$ 的长度，那么 Minkowski 不等式表达的仍然是最熟悉的几何直觉：两段位移首尾相接以后，总长度不会超过分别走这两段的长度之和。区别只在于，这里的“长度”未必是欧氏长度，而是更一般的 $p$ 长度。
...
从几何上看，Minkowski 不等式还在说明一件更深的事：当 $p >= 1$ 时，$p$ 范数的单位球是凸的。因为若 $norm(vc(x))_p <= 1$ 且 $norm(vc(y))_p <= 1$，那么对任意 $0 <= theta <= 1$，由 Minkowski 不等式与齐次性可得
$
...
因此，改变 $p$ 的过程，可以理解成单位球在菱形、圆盘、正方形之间连续变形的过程。Minkowski 不等式说的就是：这些形状虽然不同，但只要它们仍然凸，就还能担当“长度”的角色。
...
先看第一部分。单位球的凸性已经由 Minkowski 不等式说明了。中心对称性则来自齐次性，因为
$
...
第二部分说明怎样从几何形状反推出范数。定义中的 $norm(·)_K$ 称为 $K$ 的 Minkowski 泛函，也叫规范函数。它的意义是：从原点出发，沿着向量 $vc(x)$ 的方向走出去，要把单位凸体 $K$ 放大多少倍，才刚好能碰到 $vc(x)$。因此 $norm(vc(x))_K$ 不是在“算公式”，而是在测量 $vc(x)$ 相对于几何体 $K$ 的径向位置。
...
#connection[$p$ 范数确实满足三条公理][
  对 $1 <= p <= oo$，函数 $norm(·)_p$ 的非负性与齐次性都是直接可见的，而三角不等式正由 Minkowski 不等式给出。因此 $RR^n$ 上的 $p$ 范数确实都是合法的向量范数。
]
...
#connection[这些不等式为什么重要][
  Young 不等式负责处理乘积，Hölder 不等式负责处理求和，Minkowski 不等式负责处理加法，而 $p$ 范数比较定理则告诉我们不同范数之间可以怎样换算。后面研究诱导矩阵范数、条件数、误差传播以及最优化中的正则项时，这条逻辑链会不断重复出现。
]
...
+ Hölder 不等式控制双线性型 $vc(y)^"T" mt(A) vc(x)$。
+ Minkowski 不等式保证诱导范数与乘法、加法相容。
+ $p$ 范数比较定理把不同诱导范数之间联系起来。
+ Cauchy-Schwarz 不等式在矩阵层面则变成了 Frobenius 内积的不等式。
...
这一定义正是前面 Hölder 不等式的抽象版本。它在问：当 $vc(y)$ 被看成线性函数
$
...
$
  norm(·)_(p^*) = norm(·)_q
$
也就是说，前一节里的 Hölder 不等式，本质上就是“线性函数在单位球上的最大值由对偶范数给出”。
...
这个公式非常重要，因为它把“矩阵作用于向量”的问题，转成了“矩阵夹在两个向量之间”的双线性型问题。于是前面关于 Hölder 与对偶单位球的几何直觉，就可以直接搬到矩阵上来。
...
#proposition[Frobenius 内积与矩阵版 Cauchy-Schwarz][
...
这就是矩阵空间里的 Cauchy-Schwarz 不等式。它并没有什么神秘之处，因为把矩阵按元素排成一个长向量以后，Frobenius 内积就是普通内积，Frobenius 范数就是普通欧氏范数。

证明只需把矩阵拉直成向量，直接应用前一节的 Cauchy-Schwarz 不等式即可。也可以从
$
  norm(mt(A))_"F"^2 = tr(mt(A)^"T" mt(A))
$
出发，把 $RR^(m times n)$ 看成一个欧氏空间。

#proposition[Frobenius 范数与矩阵乘积][
...
几何上，这说明谱范数控制的是“每一个方向最多被拉长多少”，而Frobenius 范数控制的是“所有列向量总能量有多少”。因此用谱范数乘 Frobenius 范数，就能同时控制单方向放大率与整体能量。
...
#connection[这一节与向量不等式的对应关系][
  ...
  + Hölder 不等式对应双线性型 $vc(y)^"T" mt(A) vc(x)$ 的控制。
  + Cauchy-Schwarz 不等式对应 Frobenius 内积的控制。
  + Minkowski 不等式对应诱导范数的三角不等式与次乘性。
